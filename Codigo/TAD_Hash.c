...
void transformarMaiusculaHash(char *palav){
  int i;
...
void removeEspacoAdicionalHash(char *palav){
    char *comeco = palav;
...
void processaArquivosHash(const char *nome_arq, int doc_id){
  ...
  transformarMaiusculaHash(prim_linha);
  transformarMaiusculaHash(seg_linha);
  transformarMaiusculaHash(terc_linha);
  ...
  if(strlen(seg_linha) > 0){
    ...
    while(ingred_separado != NULL){
      ...
      transformarMaiusculaHash(ingred_separado);    // Converte o ingrediente para maiúscula
      removeEspacoAdicionalHash(ingred_separado);   // Remove espaços adicionais no início do ingrediente
